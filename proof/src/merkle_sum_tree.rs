...
pub use algorithms::{OrchardNullifier, PoseidonAlgorithm};
use error::Error;
use error::ErrorKind;
use hasher::Hasher;
use merkle_proof::MerkleProof;
use partial_tree::PartialTree;
use proof_serializers::MerkleProofSerializer;
pub use tree_item::TreeItem;
use utils::indices;
/// [`MerkleSumTree`] is a Merkle Sum Tree that is well suited for both basic and advanced usage.
///
...
#[derive(Clone)]
pub struct MerkleSumTree<T: Hasher> {
    current_working_tree: PartialTree<T>,
    history: Vec<PartialTree<T>>,
    uncommitted_leaves: Vec<T::Hash>,
}

impl<T: Hasher> Default for MerkleSumTree<T> {
    fn default() -> Self {
        Self::new()
    }
}

impl<T: Hasher> MerkleSumTree<T> {
    /// Creates a new instance of Merkle Tree. Requires a hash algorithm to be specified.
    ///
    /// # Examples
    ///
    /// ```
    /// use rs_merkle::{MerkleSumTree, algorithms::Sha256};
    ///
    /// let merkle_tree: MerkleSumTree<Sha256> = MerkleSumTree::new();
    ///
    /// let another_merkle_tree = MerkleSumTree::<Sha256>::new();
    /// ```
    pub fn new() -> Self {
        Self {
    ...
    ///
    /// ```
    /// # use rs_merkle::{MerkleSumTree, MerkleProof, algorithms::Sha256, Hasher, Error, utils};
    ...
    /// ];
    ///
    /// let merkle_tree = MerkleSumTree::<Sha256>::from_leaves(&leaves);
    /// # Ok(())
    /// # }
    ...
    // /// Similar to [`MerkleSumTree::root`], but returns a hex encoded string instead of
    // /// [`Hasher::Hash`].
    // ///
    // /// ## Examples
    // ///
    ...
    /// to be applied to the root, [`MerkleSumTree::commit`] method should be called first. To get the
    /// root of the new tree without applying the changes, you can use
    /// [`MerkleSumTree::uncommitted_root`]
    ///
    /// # Examples
    ///
    ...
    /// # use std::convert::TryFrom;
    /// # fn main() -> Result<(), Box<dyn std::error::Error>> {
    /// let mut merkle_tree = MerkleSumTree::<Sha256>::new();
    ...
    /// ```
    ///
    /// Inserts also can be chained with [`MerkleSumTree::commit`] for convenience:
    ///
    /// ```
    /// # use rs_merkle::{MerkleSumTree, MerkleProof, algorithms::Sha256, Hasher, Error, utils};
    /// # use std::convert::TryFrom;
    ...
    /// Appends leaves to the tree. Behaves similarly to [`MerkleSumTree::insert`], but for a list of
    ...
    /// Will return the same hash as [`MerkleSumTree::root`] after [`MerkleSumTree::commit`]
    ///
    /// For examples, please check [`MerkleSumTree::uncommitted_root_hex`]
    ...
    // /// Will return the same hash as [`MerkleSumTree::root_hex`] after [`MerkleSumTree::commit`]
    // ///
    // /// ### Examples
    // ///
    // /// ```
    ...
    /// Clears all uncommitted changes made by [`MerkleSumTree::insert`] and [`MerkleSumTree::append`]
