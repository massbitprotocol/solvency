...
pub trait Spec<F: FieldExt, const T: usize, const RATE: usize>: fmt::Debug + Clone {
    ...
    Sponge<F, S, Absorbing<F, RATE>, T, RATE>
{
    ...
    pub(crate) fn new(initial_capacity_element: F) -> Self {
        let (round_constants, mds_matrix, mds_inv) = S::constants();
        // for elms in round_constants.iter() {
        //     for elm in elms {
        //         Self::display(elm);
        //     }
        // }
        // println!("{:?}", round_constants.as_slice());
        // println!("{:?}", mds_matrix.as_slice());
        // println!("{:?}", mds_inv.as_slice());
